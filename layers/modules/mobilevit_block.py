...
from layers.conv_layer import ConvBlock
...
class MobileViTBlock(nn.Module):
    ...
    Args:
        ...
        in_features (int): :math:`C_{in}` from an expected input of size :math:`(N, C_{in}, H, W)`
    ...
    def unfolding(self, feature_map: Tensor) -> Tuple[Tensor, Dict]:
        ...
        batch_size, in_features, orig_h, orig_w = feature_map.shape
        ...
        reshaped_fm = feature_map.reshape(
            batch_size * in_features * num_patch_h, patch_h, num_patch_w, patch_w
        )
        # [B * C * n_h, p_h, n_w, p_w] --> [B * C * n_h, n_w, p_h, p_w]
        transposed_fm = reshaped_fm.transpose(1, 2)
        # [B * C * n_h, n_w, p_h, p_w] --> [B, C, N, P] where P = p_h * p_w and N = n_h * n_w
        reshaped_fm = transposed_fm.reshape(
            batch_size, in_features, num_patches, patch_area
        )
        # [B, C, N, P] --> [B, P, N, C]
...
class MobileViTBlockv2(nn.Module):
    ...
    def unfolding(self, feature_map: Tensor) -> Tuple[Tensor, Tuple[int, int]]:

        batch_size, in_features, img_h, img_w = feature_map.shape
        ...
        patches = patches.reshape(
            batch_size, in_features, self.patch_h * self.patch_w, -1
        )

        return patches, (img_h, img_w)

    def folding(self, patches: Tensor, output_size: Tuple[int, int]) -> Tensor:
    ...
    def resize_input_if_needed(self, x):
        batch_size, in_features, orig_h, orig_w = x.shape
